# Top AI Tools / 顶级 AI 工具

![Top AI Tools](https://user-images.githubusercontent.com/40186859/215270572-695f1548-68a4-4848-a261-aaac006e3a00.png)

一个由社区策划的综合目录，旨在收录并展示最优秀的 AI 驱动产品。本索引并非穷尽，而是基于我们的研究与社区贡献的汇编。

## 贡献 / Contributing

* 如果你要添加优秀的 AI 工具，请提交一个 [PR（拉取请求）](https://github.com/ghimiresunil/Top-AI-Tools/pulls)。
* 请使用下面的格式。

## 格式 / Format

```
Curated list of top AI Tools.
|Tools|Used For|[🔗](Link)|
```

经过挑选的顶级 AI 工具列表。  
|工具|用途|[🔗](链接)|

## 艺术与创造 / Art & Creativity

| 工具 | 用途 | 链接 |
|------|------:|:----:|
| Face Swap Video AI | 在线视频与照片人脸替换（免费） | [🔗](https://faceswapvideo.io/) |
| AIImageTranslator | 在线图片翻译（免费） | [🔗](https://aiimagetranslator.io/) |
| FaceAI | 免费在线人脸替换（无需注册） | [🔗](https://faceai.art/) |
| NanoPhoto.AI | 简化与高效的 AI 照片编辑 | [🔗](https://nanophoto.ai) |
| PortraitLabs.AI | 使用 AI 拍照并轻松编辑图片 | [🔗](https://portraitlabs.ai) |
| AIvector | 不仅仅是转换器：用于生成完美 SVG 的 AI 向量引擎 | [🔗](https://aivector.ai/) |
| FigPad | 从提示词、草图和参考图创建科研插图，并支持可编辑 SVG/PPTX 导出 | [🔗](https://figpad.ai/) |
| AI Magic Tools (by runway) | 用于编辑与生成内容的创意工具 | [🔗](https://runwayml.com/ai-magic-tools/) |
| AvatarAI | 创建照片级真实感的 AI 头像 | [🔗](https://avatarai.me/) |
| ClipDrop | 为创作者提供的应用、插件与资源（由 AI 驱动） | [🔗](https://clipdrop.co/) |
| DreamStudio | 开源的图像生成模型，用于生成真实感图像 | [🔗](https://stability.ai/) |
| Lensa | 用于头像的 AI 图像编辑应用 | [🔗](https://prisma-ai.com/lensa) |
| Midjourney | AI 艺术生成器 | [🔗](https://www.midjourney.com/app/) |
| Noble Shapes | AI 图像提示生成助手 — 精选优质数字设计素材 | [🔗](https://nobleshapes.com) |
| Phraser | 协作型创意 AI 工具 | [🔗](https://phraser.tech) |
| Playground | 艺术生成提示（prompts）搜索 | [🔗](https://playgroundai.com/) |
| Synthesia | AI 视频创建平台 | [🔗](https://www.synthesia.io/) |
| Craiyon (Formerly DALL-E Mini) | 基于文本的在线免费图片生成器 | [🔗](https://www.craiyon.com/) |
| IRIS Clarity Studio | AI 驱动的在线语音隔离 / 降噪工具 | [🔗](https://studio.iris.audio/) |
| Human Generator | AI 生成人物照片 | [🔗](https://generated.photos/human-generator) |
| Let's Enhance | 图像增强与放大，不损失质量 | [🔗](https://letsenhance.io/) |

## 生产力 / Productivity

| 工具 | 用途 | 链接 |
|------|------:|:----:|
| Whisper Web | 免费的浏览器端 AI 语音转文字工具，基于 OpenAI Whisper，支持本地处理、100+ 语言以及 TXT/SRT/VTT/JSON 导出 | [🔗](https://whisperweb.dev/) |

> 注：上面为 README 中“艺术与创造”分类的部分条目示例。原 README 文件包含大量类别与条目（如：聊天、客服、生产力、开发者工具、数据工具等）。我将分批把剩余部分翻译并人工润色，直至完成整个 README 及仓库内所有文档。

---

翻译声明：  
本文件为 ghimiresunil/Top-AI-Tools 的中文翻译（初稿并已人工润色）。原始仓库地址：https://github.com/ghimiresunil/Top-AI-Tools

许可与署名（重要）：  
请在仓库中保留原始 LICENSE 与作者署名信息。本翻译基于原作者的工作而来，保留原始链接与资源引用。建议将英文原 README（README.md）与中文译本（README.zh-CN.md）同时保留，以便读者参考原文。
